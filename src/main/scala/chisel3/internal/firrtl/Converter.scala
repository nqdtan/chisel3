...
private[chisel3] object Converter {
  ...
  private case class LoopFrame(loop: fir.Loop, outer: Queue[fir.Statement])
  ...
  def convert(cmds: Seq[Command], ctx: Component): fir.Statement = {
    ...
    def rec(acc: Queue[fir.Statement],
      ...
      if (cmds.isEmpty) {
        ...
        case Some(stmt) =>
          rec(acc :+ stmt, scope, scopeLoop)(cmds.tail)
        ...
        case None => cmds.head match {
          case WhenBegin(info, pred) =>
            ...
            rec(Queue.empty, frame +: scope, scopeLoop)(cmds.tail)
          case WhenEnd(info, depth, _) =>
            ...
            cmds.tail.headOption match {
              case Some(AltBegin(_)) =>
                ...
                rec(Queue.empty, frame.copy(when = when, alt = true) +: scope.tail, scopeLoop)(cmds.drop(2))
              case _ => // Not followed by otherwise
                ...
                rec(frame.outer :+ when, scope.tail, scopeLoop)(cmdsx)
            }
          case OtherwiseEnd(info, depth) =>
            ...
            rec(scope.head.outer :+ when, scope.tail, scopeLoop)(cmdsx)

          case LoopBegin(info, pred) =>
            val loop = fir.Loop(convert(info), convert(pred, ctx),
                                fir.EmptyStmt)
            val frameLoop = LoopFrame(loop, acc)
            rec(Queue.empty, scope, frameLoop +: scopeLoop)(cmds.tail)
          case LoopEnd(info, depth) =>
            val frameLoop = scopeLoop.head
            val loop = frameLoop.loop.copy(body = fir.Block(acc))
            val cmdsx = if (depth > 0) LoopEnd(info, depth - 1) +: cmds.tail  else cmds.tail
            rec(frameLoop.outer :+ loop, scope, scopeLoop.tail)(cmdsx)
        }
      }
    }
